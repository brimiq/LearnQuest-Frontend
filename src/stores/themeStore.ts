...
const AVAILABLE_THEMES = [
  ...
  { name: 'caramellatte', label: 'Caramel Latte', category: 'Light' },
  { name: 'silk', label: 'Silk', category: 'Light' },
  ...
  { name: 'abyss', label: 'Abyss', category: 'Dark' },
];
...
export const useThemeStore = create<ThemeState>((set) => ({
  theme: localStorage.getItem('learnquest-theme') || 'light',
  themes: AVAILABLE_THEMES,
  setTheme: (theme: string) => {
    document.documentElement.setAttribute('data-theme', theme);
    localStorage.setItem('learnquest-theme', theme);
    set({ theme });
  },
}));

// Initialize theme on load
const savedTheme = localStorage.getItem('learnquest-theme') || 'light';
